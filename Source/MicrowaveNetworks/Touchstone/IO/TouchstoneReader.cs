...
namespace MicrowaveNetworks.Touchstone.IO
{
	...
	{
		/// <summary>
		/// Returns the comments parsed from the Touchstone file.
		/// </summary>
		...
		public INetworkParametersCollection ReadToEnd()
		{
			if (!disposedValue)
			{
				...
				if (collection is null) ThrowHelper("Network Data", "No network data was read from the Touchstone file.");
				return collection!;
			}
			else throw new ObjectDisposedException(nameof(TouchstoneReader));
		}
			...
			while ((nextCharInt = reader.Peek()) != -1)
			{
				...
				switch (nextChar)
				{
					...
					case Constants.CommentChar:
						// Will never be null due to reader.Peek() called in the while loop
		...
		private static string GetTouchstoneFieldName<T>(string objectFieldName)
		{
			...
			if (f == null)
			{
				throw new ArgumentException("Field not found", nameof(objectFieldName));
			}
			else
			{
				...
				if (attr == null)
				{
					throw new ArgumentException("Field does not have TouchstoneParameterAttribute", nameof(objectFieldName));
				}
